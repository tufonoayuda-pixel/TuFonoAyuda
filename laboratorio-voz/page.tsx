...
import type { Patient, SavedSession } from '@/lib/types';
...
export default function VoiceLabPage() {
  ...
  const updatePitch = useCallback(() => {
    ...
    if (rms < 0.01) { // not enough signal
        ...
        if (freq > 80 && freq < 1000) { // Reasonable human voice range
  ...
  const handleSaveSession = async () => {
    ...
    try {
        ...
        await updateDoc(patientDocRef, {
            labSessions: arrayUnion(newSession)
        });
        toast({
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-muted-foreground">
          Herramientas de análisis acústico y vocal en tiempo real.
        </p>
      </header>
      ...
      {!hasMicPermission && (
         <Alert variant="destructive">
            ...
            <AlertDescription>
              ...
              <Button onClick={startPitchDetection} variant="link" className="pl-2">Haga clic aquí para habilitar.</Button>
            </AlertDescription>
          </Alert>
      )}
      ...
      <div className="text-center p-4 rounded-lg border bg-card space-y-4" data-tour-id="laboratorio-main">
          ...
          <div className="text-center">
              <p className="text-6xl font-bold">{pitch.note}<span className="text-4xl text-muted-foreground">{pitch.octave > 0 ? pitch.octave : ''}</span></p>
                      ...
                      <div>
                        ...
                        <div>
                          ...
                          <p className="text-2xl font-bold">{avgPitch > 0 ? `${avgPitch.toFixed(1)} Hz` : '-'}</p>
                            <p className="text-xs text-muted-foreground">{avgPitch > 0 ? `${noteFromPitch(avgPitch).note}${noteFromPitch(avgPitch).octave}` : ''}</p>
                      </div>
                  </div>
              </CardContent>
              <CardFooter>
              ...
              <CardContent className="text-center space-y-4">
                    ...
                    <div className="flex justify-center gap-2">
                      <Button onClick={() => setIsStopwatchRunning(!isStopwatchRunning)} variant="outline" size="icon">
                        ...
                        <Button onClick={() => { setIsStopwatchRunning(false); setStopwatchTime(0); }} variant="outline" size="icon">
              ...
              {recordedTimes.length > 0 && (
                  <CardFooter className="flex flex-col items-start gap-2 pt-4 border-t">
                      ...
                      <ul className="list-disc list-inside text-sm text-muted-foreground">
                          {recordedTimes.map((time, index) => <li key={index}>{time}</li>)}
                      </ul>
                        <Button variant="link" size="sm" className="p-0 h-auto" onClick={() => setRecordedTimes([])}>Limpiar</Button>
                  </CardFooter>
              )}
          </Card>
      </div>
        ...
        <Card>
            ...
            <CardContent>
                <div className='max-w-xs'>
                     <Label htmlFor="patient-select">Asignar a Paciente</Label>
